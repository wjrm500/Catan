...
class SettlingPhase(GamePhase):
    ...
    def setup_inner_frame_middle_left(self):
        canvas_mode = HexagonRendering.CANVAS_MODE_BUILD_VILLAGE if self.client_active() else HexagonRendering.CANVAS_MODE_DISABLED
    ...
    def setup_inner_frame_bottom_left(self):
        ...
        instruction_text = 'Build a village!' if client_active else 'Please wait for your turn'
    ...
    def get_introductory_text(self):
        settling_order_text = '\n\n'.join([f'{player.name} will settle {gutils.get_ordinal(i)}.' for i, player in enumerate(self.chaperone.players, 1)])
        return f"""Hi {self.chaperone.player.name}, and welcome to Catan!

A game of Catan begins with each player placing villages on two nodes, with a single road leading away from each village.

Players take it in turns to place villages and roads, with turn-taking following the “snake draft” format, such that the player who settles first will be the player who settles last.
